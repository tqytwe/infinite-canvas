...
- 用户自有 API 渠道在 Canvas 图片工作台刷新 `/v1/models` 后，必须完整展示该 Key 返回的模型。部分模型附带 `modalities`、`image_capabilities` 或 `video_capabilities` 时，不得把这些字段错误当作完整图片模型白名单。
...
- Canvas 拉取每个本地渠道的 `/v1/models` 时，图片工作台以该渠道返回的模型列表为准；能力字段仅用于已声明模型的参数约束，不得截断同一渠道的其他图片模型。
