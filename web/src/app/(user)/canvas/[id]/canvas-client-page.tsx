...
import { createVideoPosterDataUrl, resolveMediaUrl, uploadMediaFile, uploadRemoteMediaToServer, type UploadedFile } from "@/services/file-storage";
    ...
    const saveNodeAsset = useCallback(
        async (node: CanvasNodeData) => {
            ...
            if (node.type === CanvasNodeType.Video) {
                ...
                let coverUrl = "";
                try {
                    coverUrl = await createVideoPosterDataUrl(node.metadata.content);
                } catch (error) {
                    console.warn("画布视频封面提取失败，将使用视频预览兜底", error);
                }
                addAsset({
                    kind: "video",
