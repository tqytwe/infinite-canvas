...
+ [修复] 托管生图工作台新增图片分组切换，确保图片模型选择与图片专属托管 Key 使用同一平台分组。
+ [修复] 托管视频工作台完整识别 Veo 3.1 与 Seedance 2/2.5 模型，统一使用 `/v1/videos` 查询协议，并在任务查询连续网关错误时结束为明确失败，避免无限显示生成中。
+ [优化] 托管站首屏路由与 Agent、配置模块改为按需加载，并为静态脚本提供 Brotli/Gzip 压缩，减少登录后等待内容出现的时间。
+ [新增] 提示词来源新增 Freestylefly GPT Image 2 内置来源。
+ [新增] 托管模式支持极速蹬账号登录交换、AI创作空间云端画布与素材保存，并提供全站共享 30GB 持久卷配额。
+ [新增] 托管站新增站内用户文档和管理员部署文档，明确未登录只读、全站共享 30GB 空间、主平台管理边界与队列职责。
+ [新增] 极速蹬提示词广场可安全带入 AI创作空间生图工作台；提示词正文不出现在跳转 URL，用户确认生成前不会创建任务或扣费。
+ [调整] 托管模式的图片、视频和生成历史复用主平台的模型权限与异步任务接口，网页不再保存用户自己的平台 API Key。
+ [调整] 托管站品牌改为 Jisudeng，右上角移除 GitHub 入口并改为返回极速蹬主平台。
+ [修复] 托管模式会将主平台异步图片结果 URL 改写为当前域名代理，并在保存历史时转存到持久卷，避免 24 小时临时结果地址失效。
+ [修复] 托管模式刷新主平台模型目录时保留用户已选择的图片、视频、文本和音频默认模型，不再自动跳回第一个可用模型。
+ [修复] 主平台异步图片返回外部临时对象地址时，改由 Canvas 签名本域代理拉取并转存到持久卷，避免旧图像工作室对象存储/CORS 导致任务成功但结果不可见。
+ [修复] 图片工作台提交托管异步任务后立即保存 pending 历史，刷新页面可继续轮询，完成结果会转存为 Canvas 持久对象并保留失败记录。
+ [修复] 托管视频生成请求保留所选模型并透传到主平台 `/v1/videos`，避免 Agnes 视频返回 `model is required`。
+ [修复] 托管视频分组切换改用视频专属的平台会话，避免界面已选目标分组但网关仍以聊天或图片分组请求模型。
+ [修复] 视频任务完成后，工作台不会再被延迟的云端 pending 记录覆盖；已完成结果会立即显示并保留完成状态。
+ [修复] 视频历史中的成功但待落盘记录在刷新后自动重试云端保存，避免临时结果地址过期后无法恢复。
+ [修复] Agnes 视频任务在上游持续生成时保持 pending 并持续轮询，只有上游明确失败才标记生成失败，避免慢任务在前端固定超时后被误判失败。
+ [修复] 曾被旧前端超时误标失败的 Agnes 视频记录会在打开工作台时复查上游状态，并恢复为完成或继续生成。
+ [优化] 生图、视频工作台和画布节点在生成完成后先展示结果，持久化保存改为后台交付状态，慢对象读取或保存失败不再阻塞后续任务。
+ [修复] 托管模型按主平台分组保留选项值，避免同名或后续模型在选择后回落到第一个模型。
+ [调整] Docker 镜像声明 `/data/infinite-canvas` 为持久卷，并在管理员部署文档中明确 Zeabur 硬盘 ID、挂载目录和全站 30GB 上限配置。
+ [新增] 本地 30GB 媒体池增加管理员存储管理页：显示真实磁盘、用户占用排行、临时区、孤儿与隔离区，管理员清理操作均要求引用检查或明确确认。
+ [修复] 30GB 空间接近阈值时先清理过期临时文件，再回收超过保留期且无画布、资产或历史引用的媒体；引用对象不会自动删除，无法释放足够空间时会拒绝新写入。
+ [优化] 打开图片或视频工作台历史时只读取记录元数据，选中记录后才按需恢复媒体；持久卷对象读取增加缓存校验和高吞吐文件响应，避免资产增长后全量下载拖慢页面。
+ [调整] 提示词库改为本地按类型懒加载，保留原有生图来源与分类，并为生图、视频、幕布每条内置提示词提供本地缩略图完整性校验。
+ [修复] 视频工作台按模型协议提交任务：Seedance 2.5 使用 JSON `/v1/videos` 并传递 `model`、`seconds`、`resolution` 等字段，Veo/Sora 类 OpenAI 风格视频模型不再误用 multipart 表单。
